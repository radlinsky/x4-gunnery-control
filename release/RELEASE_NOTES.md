...
- ENGAGEABLE now handles large ships and stations better. Weapon range is measured to the target's bounding box instead of its center, and a line of fire blocked by a big target's own center point is re-checked against its individual surface modules. Together these stop reachable stations and capitals from reading as out of range or fully un-engageable. It is still a targeting aid, not a firing guarantee.
