...
from barrelposition_evaluator import (
    EvaluatorError,
    _barrelposition_connection,
    _native_connection_name_hash,  # noqa: F401 -- selection regression test
    joint_segments,
    load_turrets,
)
...
MACROS = {
    ...
    "turret_ter_l_beam_01_mk1_macro": ("depth4_p8_translation", 4),
...
def _chain(turret: dict[str, object]) -> list[str]:
    """Generic #166 split T = L ∘ Rx(-pitch) ∘ G ∘ Ry(yaw) ∘ H at the #164 endpoint.

    Row-vector transforms as in barrelposition_evaluator; P3 consumes this.
    """
    segments = joint_segments(turret)
    lines = [
        "        chain = {",
        f'            barrelposition_connection = "{turret["selected_connection"]}",',
    ]
    for name in ("L", "G", "H"):
        translation, rows = segments[name]
        fields = ", ".join(
            f"{key} = {{ {', '.join(_number(v) for v in values)} }}"
            for key, values in zip(("translation", "x", "y", "z"), (translation, *rows))
        )
...
def _render(report: _Report, x4_version: str, chains: dict[str, list[str]]) -> str:
    """chains: macro -> _chain() lines for every conventional combat macro."""
    if str(report["x4_version"]) != x4_version:
        raise SystemExit(
            f"census X4 version {report['x4_version']} does not match {x4_version}"
        )
    if not set(MACROS) <= set(chains):
        raise SystemExit(f"legacy macros outside the combat set: {sorted(set(MACROS) - set(chains))}")
    lines = [
    ...
    # ponytail: legacy semantic-case fields stay beside `chain` until P3 switches
    # the consumer; macros without a legacy case carry only `chain`.
    for macro in [*MACROS, *sorted(set(chains) - set(MACROS))]:
        record = _record(report, macro) if macro in MACROS else [f'    ["{macro}"] = {{', "    },"]
        lines.extend(record[:-1] + chains[macro] + record[-1:])
    lines.extend(["}", ""])
    return "\n".join(lines)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    source_sets = _mapping(args.source_set, "source-set")
    resource_sets = _mapping(args.resource_set, "resource-set")
    try:
        report = build_census(
            source_sets, resource_sets, include_source_semantic_resolutions=True
        )
        combat = sorted(
            str(entry["macro"])
            for entry in report["combat_conventional_turret_eligibility"]["macro_classifications"]
            if entry["eligibility"] == "COMBAT_CANDIDATE"
        )
        turrets = load_turrets(source_sets, resource_sets, combat)
        chains = {macro: _chain(turrets[macro]) for macro in combat}
        text = _render(cast(_Report, report), args.x4_version, chains)
    except (CensusError, EvaluatorError) as error:
        print(error, file=sys.stderr)
        return 1
    args.output.write_text(text, encoding="utf-8")
    print(f"wrote {len(chains)} turret macro geometry records to {args.output}")
    return 0
