...
rem WSLENV propagates it and an optional WSL-native extracted-source root across
rem the cmd -> wsl.exe boundary so the installer running inside WSL can see them.
set "X4GC_INSTALL_TESTLAB=1"
set "WSLENV=X4GC_INSTALL_TESTLAB:X4GC_EXTRACTED_ROOT:%WSLENV%"
