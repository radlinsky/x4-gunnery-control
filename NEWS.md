...
- **Direct-control works on M ships again.** The camera now opens normally instead of kicking you back to the console.
- **Duplicate-named turret groups are now controllable.** Previously, two groups sharing a name caused both to be greyed out.
...
- **Two modes in one compact panel: Auto-engage and Direct-control.** Auto-engage writes your staged console settings, opens the camera, and reverts everything on get-up without firing orders. Direct-control lets you pick a target element (hull, turret, shield, or engine) and arms every checked group against it.
