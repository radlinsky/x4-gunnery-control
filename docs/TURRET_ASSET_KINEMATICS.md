...
- **ANI key record** — one fixed-size stored animation-data entry belonging to an ANI descriptor. Calling it a record identifies a stored entry; it does not assign a meaning to the bytes inside it.
- **Candidate channel** — one of five ordered groups of ANI key records stored for a descriptor. Its exact X4 meaning has not yet been proved; that uncertainty is why it is called a candidate channel rather than a named animation property.
- **Byte slot** — one fixed byte position or byte range inside an ANI key record. A slot lets reports identify and compare stored data without claiming what that data means.
- **Authored animation selector** — an animation name written on a component connection, in source XML as `<animation name="...">`. Exact-name matching is a source fact; what X4 does with that match at runtime has not yet been proved.
- **Same-name ancestor coverage** — the structural fact that an ANI descriptor has an exact same-name authored animation selector either on its own component connection or on a component connection above it in the authored hierarchy. This does not prove runtime propagation from that connection to the descriptor.
- **Key-count family** — the five-number list showing how many ANI key records a descriptor stores in each candidate channel, in channel order. It is a compact count, not an interpretation of those records.
...
As a small terminology example, the offline A3 structural inventory records the exact ANI descriptors `(part_rotator, turret_active)` and `(anim_barrel, turret_active)` for this component. Each has key-count family `[2, 0, 0, 0, 0]`: two stored key records in candidate channel 0 and none in candidate channels 1–4. The exact `turret_active` authored animation selector on `Connection01` is above both descriptors' source connections in the component hierarchy, so the inventory calls this same-name ancestor coverage. This is an inference from shipped-source identities and hierarchy links; it does not give the records a meaning or prove runtime propagation.
