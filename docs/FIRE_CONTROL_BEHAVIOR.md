...
- a line of fire from the muzzle to the target is clear. The check casts to the target root; if that ray is self-blocked and the target is a modular defensible with more than one operational module (typically a station), it re-checks the line of fire to each operational surface module and counts the turret if any one module is reachable.
