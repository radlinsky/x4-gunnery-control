...
grep -Fq 'RegisterEvent("X4GunneryControl.SolutionResult", onSolutionResult)' "$main"
grep -Fq 'RegisterEvent("X4GunneryControl.SolutionBatchComplete", onSolutionBatchComplete)' "$main"
grep -Fq 'AddUITriggeredEvent("X4GunneryControl", "solution_batch_member"' "$main"
grep -Fq 'AddUITriggeredEvent("X4GunneryControl", "solution_batch_target"' "$main"
grep -Fq 'State.checkedGroups(session)' "$main"
assert_md_xpath "1" "count(//cue[@name='SolutionService'])" "solution service cue count"
assert_md_xpath "1" "count(//check_line_of_sight[@object='\$weapon'][@objectoffset='\$weapon.barrelposition'][@excludeself='false'][@useaimtarget='true'])" "solution service own-hull-aware muzzle-origin line-of-fire check count"
assert_md_xpath "1" "count(//raise_lua_event[@name=\"'X4GunneryControl.SolutionResult'\"])" "solution result event count"
assert_md_xpath "1" "count(//raise_lua_event[@name=\"'X4GunneryControl.SolutionBatchComplete'\"])" "solution batch-complete event count"
grep -Fq "SolutionService.\$targetids.{\$targetindex} + ':' + \$on + ':' + \$known + ':' + SolutionService.\$expectedmembers" "$md"
grep -Fq "\$target.relativeposition.{\$weapon}.rotation.pitch" "$md"
grep -Fq "SolutionService.\$arcmins.{\$weaponindex} * 1deg" "$md"
grep -Fq "SolutionService.\$arcmaxs.{\$weaponindex} * 1deg" "$md"
...
assert_md_xpath "1" "count(//cue[@name='SolutionMember']//do_if[contains(@value, '\$nonce == SolutionService.\$nonce')][contains(@value, 'weapons.count lt')][contains(@value, 'not SolutionService.\$weapons.indexof')])" "member nonce/count/duplicate guards"
assert_md_xpath "1" "count(//cue[@name='SolutionTarget']//do_if[contains(@value, '\$nonce == SolutionService.\$nonce')][contains(@value, 'targets.count lt')][contains(@value, 'not SolutionService.\$targets.indexof')])" "target nonce/count/duplicate guards"
...
grep -Fq 'requestSolution(pinnedID, "surface_pinned")' "$main"
grep -Fq 'requestSolutions(pageIDs, "surface_page")' "$main"
