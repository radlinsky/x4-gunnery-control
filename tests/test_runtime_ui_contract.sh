...
assert_md_xpath "1" "count(//check_line_of_sight[@excludeself='true'][@useaimtarget='true'])" "solution service line-of-fire check count"
...
grep -Fq "SolutionService.\$targetids.{\$targetindex} + ':' + \$on + ':' + SolutionService.\$expectedmembers" "$md"
grep -Fq "(\$weapon.distanceto.{\$target} + (\$target.size / 2)) lt \$weapon.maxfirerange" "$md"
assert_md_xpath "1" "count(//check_line_of_sight[parent::do_if[contains(@value, 'distanceto')]])" "range rejection wraps line-of-fire check"
...
grep -Fq 'State.filterSurfaceTargets(allSurfaces, session.surfaceTypeFilter, session.surfaceMacroFilter)' "$main"
