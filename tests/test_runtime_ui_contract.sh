...
assert_md_xpath "1" "count(//check_line_of_sight[@target='\$target'][@objectoffset='\$weapon.barrelposition'][@useaimtarget='true'][ancestor::do_if[contains(@value, 'bboxdistanceto')][contains(@value, 'aimpitch')]])" "arc and range rejection wrap current-barrel line-of-fire check"
