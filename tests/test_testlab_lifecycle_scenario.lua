...
-- Only the A8 scenario's correlated READY starts the searches. MD is modelled
-- by answering each request with X4's nearest-authored-point rule; the driver
-- must keep one correlated request outstanding, ignore stale answers, cap every
-- run at 40 samples and finish all runs with one overall COMPLETE.
do
    local function a8Events(harness)
        local events = {}
        for _, event in ipairs(scenarioEvents(harness)) do
            if event.control == "a8_target" or event.control == "a8_probe" then events[#events + 1] = event end
        end
        return events
    end
    local lines = {}
    local function count(text)
        ...
        for _, line in ipairs(lines) do
            if line:find(text, 1, true) then total = total + 1 end
        end
        return total
    end

    local other = loadHarness(localScenario("not-a8"))
    other.openFromGunnery({ label = "console", phase = "console" })
    local _, otherRequest = requestScenario(other)
    other.fix.fireEvent("X4GunneryTestLab.ScenarioReady", ready9(otherRequest, "not-a8", { spawned = 1 }))
    ...
    local debugError = DebugError
    DebugError = function(message) lines[#lines + 1] = message; debugError(message) end
    harness.openFromGunnery({ label = "console", phase = "console" })
    local _, requestId = requestScenario(harness)
    harness.fix.fireEvent("X4GunneryTestLab.ScenarioReady", ready9("stale_request", id, { spawned = 1 }))
    assert(#a8Events(harness) == 0, "an uncorrelated READY must not start the A8 searches")
    ...
    local points = {
        ...
        { -406.370, 37.752, 347.776 }, { 0, -159.479, -272.644 },
    }
    local function int(v) return string.format("%d", v >= 0 and math.floor(v + 0.5) or -math.floor(-v + 0.5)) end
    local handled, samples = 0, {}
    while true do
        local events = a8Events(harness)
        assert(#events == handled + 1, "exactly one A8 request must be outstanding")
        local event = events[#events]
        local token = event.params.token
        local run, sample = token:match("^" .. requestId .. "_(%d+)_(%d+)$")
        assert(run, "every A8 request must carry its READY request id, run and sample")
        handled = handled + 1
        harness.fix.fireEvent(event.control == "a8_target" and "X4GunneryTestLab.A8Target"
            or "X4GunneryTestLab.A8Probe", "x4gca8p:" .. requestId .. "_99_99:1:0:0:1000000000")
        assert(#a8Events(harness) == handled, "a stale A8 answer must not advance the search")
        if event.control == "a8_target" then
            assert(event.params.specId == id and sample == "0", "each run must first resolve its target")
            harness.fix.fireEvent("X4GunneryTestLab.A8Target", "x4gca8t:" .. token
                .. ":1:0x10:ship_arg_xl_carrier_02_a_macro:0:-59509:199878:591079:296986:1136737")
        else
            samples[run] = tonumber(sample)
            assert(samples[run] <= 40, "no A8 run may request a 41st sample")
            local p = { event.params.x, event.params.y, event.params.z }
            ...
            harness.fix.fireEvent("X4GunneryTestLab.A8Probe", "x4gca8p:" .. token .. ":1:"
                .. int(v[1] / length * 1e9) .. ":" .. int(v[2] / length * 1e9) .. ":" .. int(v[3] / length * 1e9))
        end
        if harness.fix.logContains("event=a8_overall") then break end
        assert(handled < 500, "the A8 searches must terminate")
    end
    assert(#a8Events(harness) == handled, "nothing may be requested after the overall result")
    assert(count("event=a8_overall") == 1 and count("runs_complete=10") == 1,
        "the overall result must be COMPLETE after all 10 runs: " .. lines[#lines])
    assert(count("result=COMPLETE") == 11 and count("points=4") == 10,
        "every run must complete and recover the four modelled aim points")
end
