...
-- Representative barrelposition endpoint identity (#164 build-pinned rule,
-- corroborated by #155 LIVE evidence). These expectations are the independently
-- recorded evidence values, not an echo of the generator's own computation.
local expected_barrelposition = {
  turret_spl_m_beam_02_mk1_macro = "con_beam_02",
  turret_spl_m_laser_02_mk1_macro = "con_laser_02",
  -- The #155 counterexample: LIVE runtime matched this endpoint and rejected
  -- con_standard_02 by about 4.938 m, so production must not take endpoints[2].
  turret_spl_m_plasma_02_mk1_macro = "con_standard_01",
  turret_ter_m_beam_02_mk1_macro = "con_beam_02",
  turret_ter_m_laser_02_mk1_macro = "con_laser_02",
}
local wrong_for_index_two = 0
for macro, expected in pairs(expected_barrelposition) do
  local record = X4GunneryTurretMuzzleGeometry[macro]
  if record.semantic_case ~= "depth3_one_key_barrel_translation" then
    fail("unexpected semantic case for " .. macro)
  end
  if record.barrelposition_connection ~= expected then
    fail(string.format("%s generated anchor %s, expected %s",
      macro, tostring(record.barrelposition_connection), expected))
  end
  if eval.production_endpoint_connection(record) ~= expected then
    fail("production did not resolve the generated anchor for " .. macro)
  end
  if record.endpoints[2].connection ~= expected then
    wrong_for_index_two = wrong_for_index_two + 1
  end
end
-- Guards the regression: if this reaches zero the suite would pass again under
-- the old unconditional endpoints[2] rule.
if wrong_for_index_two < 1 then
  fail("no #155 record discriminates the anchor from endpoints[2]")
end

-- Older semantic cases keep their historical endpoint-2 behavior for now.
for macro, record in pairs(X4GunneryTurretMuzzleGeometry) do
  if record.semantic_case ~= "depth3_one_key_barrel_translation" then
    if record.barrelposition_connection ~= nil then
      fail("unexpected explicit anchor on older record " .. macro)
    end
    local historical = record.endpoints[2] and record.endpoints[2].connection
    if eval.production_endpoint_connection(record) ~= historical then
      fail("older record changed endpoint behavior: " .. macro)
    end
  end
end
