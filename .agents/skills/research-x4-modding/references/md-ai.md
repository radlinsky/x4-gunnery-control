...
### Issue #69 prospective conventional-turret LOS origin is asset-specific
- X4: 9.00 (611726)
- Status: live-tested
- Source: `libraries/common.xsd:7253-7286,21691-21729,24111-24131,24974-25009`;
  ...
  `assets/props/WeaponSystems/energy/turret_par_l_beam_01_mk1.xml:14-23,137-282,305-307`;
  ...
  controlled Test Lab fixture `issue-69-combined-three-role-r2`, accepted FAR
  instrumentation SHA `b92d061bd7526ce9b63ea23139b7c9e5847f97a6` and repaired-NEAR SHA
  `fb8bb7214906f93989b328f32bd7be9187620d25`, game `debug.log`
- Live test: yes — accepted FAR prospective-muzzle run and repaired NEAR request
  `126694043_q2`, 2026-08-29/30, X4 9.00 build 611726
- Finding:
  - **Shipped-source:** the exact Paranid L Beam asset supplies the hierarchy and
    static animation geometry needed for the accepted construction: component
    root, Y rotator, elevation pivot and base rotation, barrel connection and
    cancelling rotation, `con_laser_02`, and the active rotator/barrel
    translations. The decoded ANI v1 key records give active rotator translation
    `[0, 6.145042419433594, 0]` and active barrel translation
    `[0, -0.23982000350952148, 27.710205078125]`. Composing those authored
    transforms gives the deployed muzzle geometry; reprojecting the downstream
    vector by the runtime target-bearing pitch and rotating the
    pivot-plus-muzzle about the authored Y rotator by the
    runtime target-bearing yaw gives the prospective target-facing muzzle used
    in the controls. No metre displacement was fitted from the live trajectory.
    This is a source-backed, per-macro construction for
    `turret_par_l_beam_01_mk1_macro`, not a generic turret transform.
  - **Inference, bounded negative API census:** the generic MD/script surface
    exposes current `weapon.barrelposition`, component transforms, an opaque
    omitted-offset LOS source, and slot transforms only when the exact slot is
    already available. It exposes neither the Beam's authored internal muzzle
    hierarchy nor an exact prospective conventional-turret firing origin. A mod
    therefore needs source-backed per-macro geometry or an explicitly labelled
    approximation. The negative search is bounded to the complete `component`,
    `weapon`, `turret`, `componentslot`, and `macroslot` property blocks in the
    cited `scriptproperties.xml`, plus the complete `find_*_slot` action census
    in the cited `common.xsd`; that X4 9.00 surface exposes no generic
    prospective-muzzle API.
  - **Live-tested — FAR:** before normal owner designation, the exact Beam's
    resting/current `barrelposition` was blocked with the firing ship included,
    while the source-backed prospective muzzle was already self-inclusive LOS
    clear to the exact FAR surface. After normal designation, the actual
    articulated `barrelposition` converged to the predicted point within about
    0.303 m. The same exact Beam then emitted `FIRED` aimed at the exact FAR
    turret surface and `HIT` that same component with `istgt=1`. This establishes
    predictive agreement for this fixture; it does not expose native controller
    semantics or model the controller's traverse trajectory.
  - **Live-tested — repaired NEAR:** the same construction, derived from the
    runtime NEAR bearing, remained blocked through a physically separate
    `ship_xen_l_terraformer_01_a_macro` blocker. Fast LOS was blocked with both
    `excludeself=false` and `excludeself=true`; all six target-local witnesses
    were blocked under both policies (0/6 clear for each). Bounding-box
    separations were 6959.875 m target-to-shooter, 3099.875 m
    blocker-to-shooter, and 2742.625 m blocker-to-target, all above the 250 m
    minimum, and `near_qualified=1`. Thus the FAR correction did not erase this
    genuine external-obstruction control.
  - **Design boundary:** this source-origin result is independent of Issue #69's
    MID target-witness/endpoint failure established by the earlier
    `issue-69-engine-straddle-r1` record below. Source origin and target point
    are two separate approximation dimensions: a better destination cannot
    repair rays launched from a transient self-masked origin, and a prospective
    origin does
    not make one representative destination exact. Preserve the bounded MID
    witness fallback as its own response.
  - **Do not generalize:** these runs do not establish native shoot-controller
    or collision semantics, applicability to other conventional-turret macros,
    or a generic X4 prospective-origin API. They also do not justify a global
    `excludeself=true` policy: the repaired NEAR external blocker remained
    blocked under both policies, while Issue #67 r11 separately established
    genuine conventional own-hull masking that self-exclusion would discard.
