...
- **Range geometry — CORRECTED 2026-08-18 against the extracted X4 9.00 tree.** The original text claimed both combat and mining AI define "this turret can engage this target" as line of sight AND `turret.distanceto.{target} + (target.size/2) < turret.maxfirerange`. That attributed the MINING form to combat. Shipped source shows two different range tests:
  - **Boundary, 2026-09-24:** this capital-combat test is a ship-positioning heuristic (hold position / reposition / retarget a module), not the engine's turret fire gate, and `bboxdistanceto` measures this box to the other's origin minus the other's radius, not box-to-box. See [turret-fire-range-gate.md](turret-fire-range-gate.md) for the native pre-fire range rule.
