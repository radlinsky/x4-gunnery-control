...
| Rank-1 one-key `turret_active` channel-0 settled value | [turret-rank1-one-key-channel0-settled.md](turret-rank1-one-key-channel0-settled.md) | X4 9.00 source-resolved constant settled additive local translation for the three `plasma_02_mk1` components' single-key `part_barrel` record; single-frame 50–50 active state, STEP interpolation, `gun_firing` recoil overlaps the active frame |
