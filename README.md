...
checked groups, not every turret on the ship. When you get up, Gunnery Control
restores the last settings you made permanent with **Update turret behavior**.

**Direct-control**: you tell the checked groups exactly what to shoot.
The **Select Engagement Target** menu lists ships and stations in radar range,
shows each candidate's S/M/L/XL class (or Station), localized type, distance and
**N / total ENGAGEABLE** count, and lists fully ENGAGEABLE candidates first.
Click a ship or station to engage its hull immediately, or switch to one of its
surface elements.

**Surface-element browser.** While engaged, the upper-left panel keeps the
current hull/surface aim point pinned and lets you switch between the parent hull
and operational turrets, shields and engines. Filter surfaces by
**Turret / Shield / Engine** and by installed equipment. Alternatives are ordered
largest-first (XL → L → M → S → XS), then by type and distance, and are paged 20
at a time. Every listed surface shows distance and its own ENGAGEABLE count; the
pinned aim point also shows shield and hull status. You can refresh manually or
enable an optional 10-second automatic refresh.

**Engageable ratio.** Gunnery Control shows an **N / total ENGAGEABLE** count for
each evaluated target or surface. This is a mod-computed geometry check — not a
guarantee the turret will fire. The numerator counts selected operational turrets
that currently pass Gunnery Control's geometry check: known traverse arc covering
the aim direction, target within weapon range, and a clear muzzle-to-target line
of fire past your own ship. Unknown-arc turrets stay in the denominator and are
reported separately as **UNKNOWN**. Readiness, fire authorization, an intercept
solution, and actual firing are all separate states that this ratio does not
measure.
...
- **The ENGAGEABLE ratio is a targeting aid, not a firing guarantee.** A turret
  can pass the geometry check and still hold fire because it is not ready, lacks
  authorization, or cannot solve the intercept. Conversely, **UNKNOWN** turrets
  may still fire when their arc data are unavailable even though the mod cannot
  classify their geometry.
